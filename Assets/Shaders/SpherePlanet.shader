...
Shader "EVE/Planet"
{
	Properties
	{
		...
		_DetailOpacityMin ("Detail Min Opacity", Range(0,1)) = 0.1
		_DetailOpacityMax ("Detail Max Opacity", Range(0,1)) = 1.0
	...
	SubShader
	{
		...
		Pass
		{
			...
			float _DetailDist;
			float _DetailOpacityMin;
			float _DetailOpacityMax;
			float _MinLight;
			float _Albedo;
			...
			fixed4 frag (v2f IN) : COLOR
			{
				...
				//half3 normT = UnpackNormal(GET_CUBE_MAP_1(_BumpMap, IN.sphereNormal)); // wrong highlight position
				// Don't know. Just copied UnpackNormal code here. "Sometimes it doesn't just work" i guess?
				half4 normPacked = GET_CUBE_MAP_1(_BumpMap, IN.sphereNormal);
				half3 normT;
				#if ( defined(SHADER_API_GLES) || defined(SHADER_API_MOBILE) )
					normT = normPacked.xyz * 2 - 1;
				#else
					normT.xy = normPacked.wy * 2 - 1;
					normT.z = sqrt(1 - normT.x*normT.x - normT.y * normT.y);
				#endif
				...
				// decrease detail opacity with brightness
				half detailOpacity = _DetailOpacityMax - (_DetailOpacityMax - _DetailOpacityMin)*0.333333*(main.r + main.g + main.b);
				// not show details on the ocean
				#ifdef OCEAN_ON
					color = main.rgba * lerp(lerp(1, detail.rgba, detailOpacity), 1, lerp(detailLevel, 1, main.a));
				#else
					color = main.rgba * lerp(lerp(1, detail.rgba, detailOpacity), 1, detailLevel);
				#endif
				#ifdef CITYOVERLAY_ON
